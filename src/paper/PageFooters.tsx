...
import { PageMarginText } from './PageMarginText';
...
export function PageFooters({ doc, root }: { doc: Doc; root: RefObject<HTMLDivElement | null> }) {
  ...
  return pages.map((page, i) => {
    ...
    return createPortal(<Fragment>
      {footer.enabled && <footer className={`page-folio${footer.right ? ' page-folio--right' : ''}`} style={{ color: footerInk(doc.design.paperBg), fontFamily: fontStack(footer.fontFamily), fontSize: `${footer.fontSize}pt`, paddingBottom: `${footerBottomOffset(doc)}mm`, height: `calc(${footerBottomOffset(doc)}mm + ${footer.fontSize}pt + 2mm)` }} aria-label={`Page ${footer.number}`}>
      ...
      <PageMarginText doc={doc} index={i} />
    </Fragment>, page, `folio-${i}`);
  });
}
