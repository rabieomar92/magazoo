...
export function FooterSection() {
  ...
  return <Section title="Page footer">
    ...
    <TypographyControl group="footer" order={10}><LabeledSelect editorTarget="footer-font" label="Footer font" value={footer.fontFamily} options={fontOptions(ALL_FONTS)} onChange={v => update(d => { d.footer = { ...d.footer, fontFamily: v }; })} /></TypographyControl>
    <TypographyControl group="footer" order={20}><LabeledNumber editorTarget="footer-font-size" label="Footer size" unit="pt" value={footer.fontSize} min={5} max={18} step={0.5} onChange={v => update(d => { d.footer = { ...d.footer, fontSize: Math.max(5, Math.min(18, v)) }; })} /></TypographyControl>
