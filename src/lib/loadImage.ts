...
export function loadImage(file: File): Promise<LoadedImage> {
  return new Promise((resolve, reject) => {
    if (!file.type.startsWith('image/')) {
      reject(new ImageLoadError('The file is not an image.'));
      return;
    }
    if (file.size > MAX_IMAGE_BYTES) {
      const mb = Math.round(MAX_IMAGE_BYTES / (1024 * 1024));
      reject(new ImageLoadError(`Image is too large (max ${mb} MB).`));
      return;
    }
    ...
    reader.onload = () => {
      ...
      const img = new Image();
      img.onerror = () => reject(new ImageLoadError('Image is corrupted or unreadable.'));
      img.onload = () =>
