...
export function cssVars(d: Design, templateId?: TemplateId): Record<string, string> {
  ...
  return {
    ...
    '--page-bg-opacity': String(d.pageBackgroundOpacity ?? 1),
    '--page-w': `${PAGE_W}mm`,
    '--page-h': `${PAGE_H}mm`,
    '--margin': `${d.margin}mm`,
    '--gutter': `${d.gutter}mm`,
    ...
    // The header has its own gutter above it, hence firstPageTop - gutter: its
    // visible top then lands exactly on the requested coordinate. The hero or
    // masthead slot always wins if it is naturally taller.
    '--hero-slot-h': `max(${nativeHeroSlot}, calc(${firstPageTop}mm - ${d.gutter}mm))`,
    '--topbar-offset': `${topBarOffset}mm`,
    '--first-page-top': `${firstPageTop}mm`,
