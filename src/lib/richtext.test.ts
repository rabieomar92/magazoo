...
import { openMarkers, parseRuns, runsToHtml, splitRichTextAt, wrapSelection } from './richtext';
...
describe('runsToHtml', () => {
  ...
  it('keeps a drop cap inside the opening rich-text style', () => {
    expect(runsToHtml('**Bold start**', true)).toBe(
      '<strong><span class="drop-cap">B</span>old start</strong>',
    );
  });
});
...
describe('splitRichTextAt', () => {
  it('closes and reopens formatting that crosses the image caret', () => {
    const source = 'Before **bold middle text** after';
    const { head, tail } = splitRichTextAt(source, source.indexOf('middle'));
    expect(head).toBe('Before **bold **');
    expect(tail).toBe('**middle text** after');
    expect(openMarkers(head)).toEqual([]);
    expect(openMarkers(tail)).toEqual([]);
  });

  it('moves a caret inside inline math to the end of the expression', () => {
    const source = 'Before $E=mc^2$ after';
    expect(splitRichTextAt(source, source.indexOf('mc')).head).toBe('Before $E=mc^2$');
  });
});
