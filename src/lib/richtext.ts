...
/** Split the first user-visible grapheme from a plain-text run. `Array.from`
 * keeps surrogate-pair characters whole; the following Unicode marks keep an
 * Arabic/Jawi vowel mark attached to its base letter. A real element is used
 * for the cap (rather than `::first-letter`) because Chromium can repeat a
 * generated first letter at multicolumn fragment boundaries in RTL text. */
function takeDropCap(text: string): { leading: string; cap: string; rest: string } | null {
  const leading = text.match(/^\s*/u)?.[0] ?? '';
  const body = text.slice(leading.length);
  const first = Array.from(body)[0];
  if (!first) return null;
  let end = first.length;
  while (end < body.length) {
    const mark = Array.from(body.slice(end))[0];
    if (!mark || !/^\p{M}$/u.test(mark)) break;
    end += mark.length;
  }
  return { leading, cap: body.slice(0, end), rest: body.slice(end) };
}
...
export function runsToHtml(src: string, dropCap = false): string {
  ...
  return parseRuns(src)
    .map((r) => {
      ...
      let html: string;
      const first = capPending ? takeDropCap(r.text) : null;
      if (first) {
        capPending = false;
        html = `${escapeHtml(first.leading)}<span class="drop-cap">${escapeHtml(first.cap)}</span>${escapeHtml(first.rest)}`;
      } else {
...
/** Split stored rich text at an editor caret without breaking a marker token
 * or an inline-math expression. Formatting that crosses the cut is closed on
 * the head and reopened on the tail, exactly like pagination's word split. */
export function splitRichTextAt(src: string, requestedOffset: number): { head: string; tail: string } {
  let offset = Math.max(0, Math.min(src.length, requestedOffset));

  // A figure cannot live inside a single TeX expression. If the caret is in
  // one, place it immediately after that expression instead.
  let mathOpen = src.indexOf('$');
  while (mathOpen !== -1) {
    const mathClose = src.indexOf('$', mathOpen + 1);
    if (mathClose === -1) break;
    if (offset > mathOpen && offset <= mathClose) {
      offset = mathClose + 1;
      break;
    }
    mathOpen = src.indexOf('$', mathClose + 1);
  }

  // Never cut between the two characters of ** or __.
  if (offset > 0 && offset < src.length) {
    const pair = src.slice(offset - 1, offset + 1);
    if (pair === '**' || pair === '__') offset += 1;
  }

  let head = src.slice(0, offset);
  let tail = src.slice(offset);
  if (head && tail) {
