    ...
    <div className="issue-body">
      ...
      <section className="issue-card">
        ...
        {!ready
          ...
          : <p className="issue-help">{entries.length} {entries.length === 1 ? 'entry' : 'entries'} in the contents spread · {physicalPages} physical pages{isFinalized ? ' · Finalised' : ''}. There’s no separate preview to check here — exporting produces exactly what each article’s own editor shows, because it’s the same page engine.</p>}
        ...
        {ready && <IssueRenderTarget plan={plan} entries={entries} assignments={assignments} items={items} showAll={showAll} magazineName={data.project.name} design={contentsDesign} pagesRef={pagesRef} onOverflow={setContentsOverflow} />}
      </section>
    </div>
