...
it('places every article through the editor’s own preview, in plan order, without touching the stored documents', () => {
  ...
  const render = (all: boolean, order = plan.order) => act(() => root.render(<IssueRenderTarget plan={{ ...plan, order }} entries={entries} assignments={assignments} items={items} showAll={all} magazineName="Magazoo!" pagesRef={pagesRef} onOverflow={() => {}} />));
